...
use regex::Regex;
use dc_minecraft::{parse, FunctionIdentifier, execute, Command};
use dc_minecraft::Function;
use dc_minecraft::execute::{Game, Datapack};
use colored::*;
...
lazy_static! {
    static ref PATH_PATTERN: Regex = Regex::new(r"^data/(\w+)/functions/(\w+).mcfunction$").unwrap();
}
